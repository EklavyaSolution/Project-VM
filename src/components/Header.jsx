...
const Header = () => {
    ...
    return (
        <nav className={`w-full max-[700px]:pl-4 max-[1200px]:pl-7 pr-4  min-[700px]:pr-7 min-[1200px]:pr-2 min-[1245px]:pr-4 min-[1278px]:pr-7 flex items-center -py-8 transition-all duration-300 delay-200 ease-out justify-between sticky left-0 bg-black  z-[999] ${visible ? "-top-[100%]" : "top-0"}`}>
            ...
            <div className='hidden min-[1200px]:flex items-center gap-5 min-[1245px]:gap-6 font-semibold text-[15px] text-white'>
                {navlinks.map((item, index) => {
                    return <div key={index} className='relative group'>
                        ...
                        {item?.children && <div className='bg-black border border-[#333] p-4 rounded flex-col gap-5 absolute top-11 -left-1 hidden group-hover:flex hover:flex'>
            ...
            <div className={` ${navActive ? "pointer-events-auto" : "pointer-events-none"} fixed block p-5 min-[1200px]:hidden top-0 left-0 w-screen h-screen`}>
                <div className={`absolute top-0 min-[700px]:top-5 pointer-events-auto w-full max-w-[350px] text-white h-screen min-[700px]:h-[95vh] min-[700px]:my-auto -mr-1 min-[700px]:mr-5 min-[700px]:rounded-xl bg-[#263238] scale-[1.02] transition-all duration-500 ease-in-out  ${navActive ? "right-0" : "-right-[100%]"}`}>
                    <Image src={close} alt='Close Icon' className='absolute top-6 right-8 min-[700px]:right-5  w-12 p-2 cursor-pointer' onClick={() => setNavActive(false)} />
                    ...
                    <div className='mt-[88px]'>
                        ...
                        <ul className=' text-center'>
                            {navlinks.map((item, index) => {
                                return <li key={index}><Link href={item.link} className='mb-5 inline-block opacity-55 transition hover:opacity-100' onClick={() => setNavActive(false)}><h3 className='font-medium text-2xl'>{item.name}</h3></Link></li>
                            })}
                        </ul>
                    </div>
                </div>
